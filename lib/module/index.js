...
module.exports = function(fm) {
  ...
  proto.render = function() {
    ...
    var el = this.el;
    var classes;
    if (el && el.tagName.toUpperCase() === this.tag.toUpperCase()) {
      if(this.isSVG) {
        var html = this._innerHTML();
        ...
        for(var i=0, len=cn.length; i<len; i++) {
          var newNode = el.ownerDocument.importNode(cn[i], true);
  ...
  proto._setSVGEl = function(el) {
    var existing = this.el;
    var parentNode = existing && existing.parentNode;

    // If the existing element has a context, replace it
    if (parentNode) {
			var newNode = parentNode.ownerDocument.importNode(el, true);
			parentNode.replaceChild(newNode, existing);
		}
